...
if input_data == 'cam':
	...
	for i, d in enumerate(dets):
		...
		if idx[i]==0 or idx[i]==1 or idx[i]==2 or idx[i]==3 or idx[i]==4:
			...
			if label!='UNKNOWN':
				# cv2.putText(frame,identity+'='+str(round((label_prob*100),2))+'%',(d.left(),  d.top()-10),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),2)
				cv2.putText(image,identity+'='+str(round((label_prob*100),2))+'%',(d.left(),  d.top()-10),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),2)
			else:
				cv2.putText(image,'???',(d.left(),  d.top()-10),cv2.FONT_HERSHEY_SIMPLEX,0.8,(0,255,0),2)
